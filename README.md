...
Se necesita de un segundo modelo de inferencia porque la forma en que aprende el modelo y en cómo se elaboran las predicciones son distintas. Durante el entrenamiento, se proporcionan pares de secuencias de entrada y salida esperadas. El modelo de entrenamiento se entrena para minimizar la diferencia entre las salidas generadas por el decodificador y las salidas esperadas. Esto se hace ajustando los pesos del modelo a través del algoritmo de backpropagation.

Una vez que el modelo ha sido entrenado, se puede utilizar para generar predicciones o inferencias en nuevas secuencias de entrada. Sin embargo, durante la inferencia, no tenemos las salidas esperadas para calcular el error y ajustar los pesos del modelo. En su lugar, necesitamos utilizar el modelo entrenado para generar secuencias de salida basadas en las secuencias de entrada.

Aquí es donde entra en juego el modelo de inferencia separado. El modelo de inferencia utiliza los mismos componentes (codificador y decodificador) que el modelo entrenado, pero se utiliza en un contexto diferente. Durante la inferencia, el decodificador genera secuencias de salida paso a paso utilizando la representación latente del codificador y las salidas generadas previamente. Sin embargo, a diferencia del entrenamiento, no se tiene la secuencia de salida esperada para comparar. En cambio, se utilizan estrategias como la generación basada en muestras o la búsqueda de haz (beam search) para obtener la secuencia de salida final.
...
Al principio dividimos los datos en lotes de 30000 muestras, debido a que queríamos hacer los lotes lo más grandes posible que permitiese el hardware. Queríamos mejorar la eficiencia del entrenamiento aventajándonos de la capacidad de procesado paralelo en la GPU, por otro lado también queríamos una mayor generalización y estimaciones de gradiente más suaves, esto es debido a que lotes más grandes son más representativos de todo el dataset en cuanto a variedad de muestras, comparado con lotes más pequeños o ejemplos individuales. Con esta implementación queríamos actualizaciones más estables y solucionar el problema de memoria.

Optamos también por crear un data loader para generar lotes de datos para el entrenamiento e ir agregándolos de a poco a memoria. No obstante, el problema persistía y si queríamos ejecutar el modelo con lotes de miles de datos, obteníamos errores de memoria. Es por ello que optamos por reducir la cantidad de datos y en lugar de usar las 140000 frases, usar simplemente 80000 (los tamaños de los lotes ya no son de 30000, pasan a ser de 20000). 
...
Al ajustarse demasiado el modelo a los datos de nuestro conjunto de training, barajamos diferentes opciones para de alguna manera, simplificar nuestro modelo: drop out, disminución del latent dim, regularización y batch normalization (la gráfica combinada con estos dos últimos no aparecía en la presentación debido a que, como se mostrará a continuación, tienen menor impacto que la disminución de latent_dim y el dropout. Aún así queríamos dejar constancia de que se habían tenido en cuenta estas dos opciones).
...
En cuanto a la regularización y el batch normalization ambos, como los anteriores mencionados, sirven para gestionar el overfitting. En nuestro caso decidimos aplicar una regularización L2 (daba mejores resultados que una L1) y ayuda a controlar la complejidad del modelo y limita el grado en que los pesos se adaptan a los datos de entrenamiento. El batch normalization, por otro lado, es una técnica que normaliza las salidas de una capa oculta al calcular la media y la varianza dentro de cada lote (batch) durante el entrenamiento. Esto ayuda a estabilizar la distribución de las activaciones de las capas y acelerar el entrenamiento.
...
<img width="480" alt="image" src="https://github.com/DCC-UAB/xnap-project-ed_group_03/assets/133141608/7889de71-c28f-45e9-bd4d-f5797ff7b2ca">

Se puede observar en la gráfica que la modificación que presenta mejores resultados es la disminución de la complejidad del modelo (reducir latent_dim). Con dropout vemos que se obtiene un sobreajuste al final (empieza a aumentar la función de pérdida de validación). Los resultados presentados con batch normalization y la regularización L2 son buenos y no presentan sobreajuste, sin embargo, como la gráfica del regularizador tiene como un patrón ondulado y la de batch normalization converge antes, los hemos decidio obviar. Es por ello que, en el modelo de las últimas ejecuciones tuvimos en cuenta dropout y la disminución de latent_dim (el gráfico se muestra al final del siguiente apartado).
...
La función de pérdida en la validación se puede ver cómo aumenta pasadas las 30 épocas y como la función de precisión de validación converge antes que la función de precisión del conjunto de entrenamiento cuando el learning rate es fijo. Cuando le añadimos un schedule, solucionamos estos problemas, como se observa en la línea gris.
...
Como se puede ver en el gráfico de más a la derecha y como ya se ha comentado anteriormente, al inicio se sobreentrenaba cada bloque provocando caídas en las funciones de precisión y aumentos en las funciones de pérdida (se sobreajustaba a los datos de ese bloque). Aplicando también todos los cambios mencionados: dropout, disminución de latent_dim, learning rate schedule y optimizer (simplemente cambiamos RMSprop por Adam) se ha obtenido la gráfica morada, que cómo se puede observar, es mucho mejor que la primera gráfica.
...
Vemos que sucede lo mismo para el caso del finlandés, una palabra sencilla la predice bien, una más elaborada ya no (la traducción de la segunda al castellano sería: 'me duele la pierna', la cual, obviamente no es la traducción correspondiente a: 'My name is'). Cabe añadir, que estos resultados al menos nos dan en el idioma correcto, y que estas tienen sentido en el respectivo idioma, ya que las primeras predicciones ni formaban parte de ningún idioma ni tenían significado alguno.
